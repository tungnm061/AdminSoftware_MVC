...
$(document).ready(function () {
    ...
    $('#btnCreateWorkingNote').click(function () {
        InitChildWindowModal('/sale/Home/WorkingNote', 550, 330, "Thêm tiến độ thực hiện công việc");
    });
    ...
    $('#btnEditWorkingNote').click(function () {
        ...
        InitChildWindowModal('/sale/Home/WorkingNote?id=' + id, 550, 330, "Cập nhật tiến độ thực hiện công việc");
    });
    $('#btnDeleteWorkingNote').click(function () {
        ...
        if (id != null && id != typeof undefined) {
            ...
            $.ajax({
                type: 'POST',
                url: '/sale/Home/DeleteWorkingNote',
                dataType: "json",
                data: JSON.stringify({
                    id: id
                }),
    ...
    $("#btnSaveAdd").click(function () {
        ...
        if (status === '5' && ($("#Explanation").val() == null || $("#Explanation").val().trim() === "")) {
            InitChildWindowModal('/sale/Home/Explanation', 600, 300, 'Giải trình công việc');
            return;
        }
                ...
                success: function (response) {
                    ...
                    if (response.Status === 0) {
                        ...
                        $.msgBox({
                            ...
                            buttons: [{ value: "Đồng ý" }],
                            success: function () {
                                $("#scdMain").data("kendoScheduler").dataSource.read();
                                GridCallback('#grdMainDangTh');
                                GridCallback('#grdMainDuocGiao');
                                CloseWindowModal();
                            }
                        });
                    }
