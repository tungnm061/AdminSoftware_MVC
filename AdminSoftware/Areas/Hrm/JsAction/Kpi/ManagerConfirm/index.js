...
$(document).ready(function () {
        ...
        dataSource: {
            transport: {
                read: function (options) {
                    ...
                    {
                        type: 'POST',
                        url: '/hrm/ManagerConfirm/WorkDetails',
                        dataType: "json",
                        data: JSON.stringify({
                            action: $("#Action").val(),
    ...
    $('#btnEditAll').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '/hrm/ManagerConfirm/SaveAll',
            data: JSON.stringify({
    ...
    $('#btnEdit').click(function () {
        ...
        if ($("#Action").val() === '3') {
            ...
            InitWindowModal('/hrm/ManagerConfirm/ConfirmFinish?id=' + id + '&workType=' + workType, false, 800, 545, 'Xác nhận công việc đã hoàn thành', false);
        }
        if ($("#Action").val() === '4') {
            $("#btnSaveCancel").prop("disabled", false);
            $("#btnSaveAdd").prop("disabled", true);
            InitWindowModal('/hrm/ManagerConfirm/ConfirmFinish?id=' + id + '&workType=' + workType, false, 800, 545, 'Cập nhật công việc đã hoàn thành', false);
        }
        if ($("#Action").val() === '5') {
            InitWindowModal('/hrm/ManagerConfirm/CancelFinish?id=' + id + '&workType=' + workType, false, 800, 545, 'Chi tiết công việc chưa hoàn thành', false);
        }
    });
});