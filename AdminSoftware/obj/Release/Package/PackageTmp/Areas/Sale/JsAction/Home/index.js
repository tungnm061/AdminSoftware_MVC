$(document).ready(function () {
    $(document).on("change", "input[type='radio'][name=status]", function () {
        ...
        var dataSource = new kendo.data.SchedulerDataSource({
            ...
            transport: {
                read: {
                    url: "/sale/Home/WorkDetailSchedulers?month=" + (new Date().getMonth() + 1) + "&year=" + (new Date().getFullYear()) + "&viewStyle=" + $("#CheckAction").val(),
                    dataType: "json"
                }
            },
            schema: {
    ...
    $("#grdMainTheoDoi").kendoGrid({
        dataSource: {
            transport: {
                read: '/sale/Home/AssignWorkFollowBys',
                dataType: "json"
            },
            schema: {
                model: {
    ...
    $('#btnDeXuat').click(function () {
        ...
        InitWindowModal('/sale/Home/EditWork?id=' + id + '&workType=' + workType, false, 800, 550, 'Thực hiện công việc', false);
    });
    $("#grdMainGiaoViec").kendoGrid({
        dataSource: {
            transport: {
                read: '/sale/Home/AssignWorkCreateBys',
                dataType: "json"
            },
            schema: {
                model: {
    ...
    $("#grdMainDuocGiao").kendoGrid({
        dataSource: {
            transport: {
                read: '/sale/Home/AssignWorkAssignBys',
                dataType: "json"
            },
            schema: {
                model: {
    ...
    $("#grdMainDangTh").kendoGrid({
        dataSource: {
            transport: {
                read: '/sale/Home/WorkNeedCompletes',
                dataType: "json"
            },
            schema: {
                model: {
    ...
    $('#btnCapNhatAssignWork').click(function () {
        ...
        InitWindowModal('/sale/Home/EditWork?id=' + id + '&workType=' + 4, false, 800, 550, 'Thực hiện công việc', false);
    });
    ...
    $('#btnGiaiTrinh').click(function () {
        ...
        InitWindowModal('/sale/Home/WorkDetailExplanation?id=' + id + '&workType=' + workType, false, 600, 300, 'Giải trình công việc');
    });
    $('#btnGiaoViec').click(function () {
        InitWindowModal('/sale/Home/AssignWork', false, 680, 370, 'Giao việc', false);
    });
});
